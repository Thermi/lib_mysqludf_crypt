...
--datadir="${this_dir}/mysqld_base_dir/mysqld_data_dir"
SYSTEMD_UNIT=$(systemd-run --user -p Type=notify -E LANG=C -E LD_PRELOAD=/usr/lib/libjemalloc.so \
mysqld ${MARIADB_START_ARGS} 2>&1  | python -c 'import sys; print(sys.stdin.readline().split(" ")[-1])')
echo "$SYSTEMD_UNIT" > "${this_dir}/curr_systemd_unit"
