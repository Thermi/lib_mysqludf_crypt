...
DELIMITER //;
CREATE OR REPLACE FUNCTION UDF_SHA256 (salt VARCHAR(256), password VARCHAR(256)) RETURNS VARCHAR(256) DETERMINISTIC
RETURN lib_mysqludf_crypt_sha256(CONCAT(salt, password));
//

CREATE OR REPLACE FUNCTION UDF_SHA384 (salt VARCHAR, password VARCHAR) RETURNS VARCHAR DETERMINISTIC
